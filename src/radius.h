...
#define _forget(X) if (X) {memset(X, 0, strlen(X));free(X);X = NULL;}
#ifndef _drop
#define _drop(X) if (X) {free(X);X = NULL;}
#endif

#define _SUCCESS 0
#define _ABORT -1
#define _AUTHINFO_UNAVAIL -2
#define _IGNORE -3
#define _HOST_NOT_FOUND -4
#define _HOST_NOT_ANSWERED -5
#define _WRONG_PACKED -6

#define _GEN_ERROR -10
